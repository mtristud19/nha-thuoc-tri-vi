...
class NhaSanXuatDetailAPIView(APIView):

    def get_object(self, ma_nsx):
        try:
            return NhaSanXuat.objects.get(ma_nsx=ma_nsx)
        except NhaSanXuat.DoesNotExist:
            return None

    # 3. Retrieve
    def get(self, request, ma_nsx, *args, **kwargs):
        
        nhasanxuat_instance = self.get_object(ma_nsx)
        if not nhasanxuat_instance:
            return Response(
                {"res": "Object - Nha San Xuat does not exists"},
                status=status.HTTP_400_BAD_REQUEST
            )
        serializer = NhaSanXuatSerializer(nhasanxuat_instance)
        return Response(serializer.data, status=status.HTTP_200_OK)
...
class DonViSanPhamDetailAPIView(APIView):
    # # add permission to check if user is authenticated
    # permission_classes = [permissions.IsAuthenticatedOrReadOnly]

    def get_object(self, ma_dvsp):
        try:
            return DonViSanPham.objects.get(ma_dvsp=ma_dvsp)
        except DonViSanPham.DoesNotExist:
            return None

    # 3. Retrieve
    def get(self, request, ma_dvsp, *args, **kwargs):
        
        donvisanpham_instance = self.get_object(ma_dvsp)
        if not donvisanpham_instance:
            return Response(
                {"res": "Object - Don Vi San Pham does not exists"},
                status=status.HTTP_400_BAD_REQUEST
            )

        serializer = DonViSanPhamSerializer(donvisanpham_instance)
        return Response(serializer.data, status=status.HTTP_200_OK)
